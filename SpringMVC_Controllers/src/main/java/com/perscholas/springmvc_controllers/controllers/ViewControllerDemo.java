...
public class ViewControllerDemo {
	...
	public String getStudent(@PathVariable Integer id, Model model) {
		...
		try
		{
			Student student = restTemplate.getForObject("http://localhost:8080"
					+ "/SpringMVC_Controllers/student/" + id, Student.class);
	...
	public String displayStudent(Model model) {
		ResponseEntity<Map<Integer, Student>> response = restTemplate.exchange("http://localhost:8080/SpringMVC_Controllers/getAllStudents",HttpMethod.GET,null,new ParameterizedTypeReference<Map<Integer,Student>>(){});
