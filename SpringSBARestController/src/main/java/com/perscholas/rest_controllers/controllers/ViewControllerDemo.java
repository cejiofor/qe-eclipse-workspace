...
public class ViewControllerDemo {
	...
	/* The showControllerView() method is a basic controller method which returns 
	 * a string that represents a JSP. */
	@GetMapping("/")
	...
	public String getStudent(@PathVariable Integer id, Model model) {
		...
		try
		{
			Student student = restTemplate.getForObject("http://localhost:8080"
					+ "/SpringSBARestController/student/" + id, Student.class);
	...
	public String displayStudent(Model model) {
		ResponseEntity<Map<Integer, Student>> response = restTemplate.exchange("http://localhost:8080/SpringSBARestController/getAllStudents",HttpMethod.GET,null,new ParameterizedTypeReference<Map<Integer,Student>>(){});
