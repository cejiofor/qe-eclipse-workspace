...
Alter TABLE StudentCourse MODIFY EndDate DATE NOT NULL;
--QUESTION???????
...
*/
Select firstname as `First Name`, lastname as `Last Name`, Round(avg(progress),10) as `Average Progress`
from student s
join studentcourse sc
on s.id = sc.studentId
group by s.id
...
-- QUESTION SORT ORDER for SAME NUMBER
